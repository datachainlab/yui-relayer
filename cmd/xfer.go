...
func xfersendBulk(ctx *config.Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "transfer-bulk [path-name] [src-chain-id] [dst-chain-id] [amount] [dst-addr] [count]",
		Short: "Initiate a transfer from one chain to another in bulk",
		Long: "Sends the first step to transfer tokens in an IBC transfer." +
			" The created packet must be relayed to another chain",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("not found cound")
			}
			...
			case toHeightOffset > 0 && toTimeOffset > 0:
				return fmt.Errorf("cannot set both --timeout-height-offset and --timeout-time-offset, choose one")
			case toHeightOffset > 0:
				return core.SendTransferMsgs(c[src], c[dst], amount, dstAddr, toHeightOffset, 0, count)
			case toTimeOffset > 0:
				return core.SendTransferMsgs(c[src], c[dst], amount, dstAddr, 0, toTimeOffset, count)
			case toHeightOffset == 0 && toTimeOffset == 0:
				return core.SendTransferMsgs(c[src], c[dst], amount, dstAddr, 0, 0, count)
			default:
				return fmt.Errorf("shouldn't be here")
			}
		},
