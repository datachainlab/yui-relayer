...
func SendTransferMsg(src, dst *ProvableChain, amount sdk.Coin, dstAddr fmt.Stringer, toHeightOffset uint64, toTimeOffset time.Duration) error {
	defer utils.Track(time.Now(), "SendTransferMsg()", nil)
...
// For bulk test
func SendTransferMsgs(src, dst *ProvableChain, amount sdk.Coin, dstAddr fmt.Stringer, toHeightOffset uint64, toTimeOffset time.Duration, count int) error {
	defer utils.Track(time.Now(), "SendTransferMsgs()", nil)
